...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'file_path',
            type=str,
            help='Path to JSON file with ingredients data'
        )
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for item in ingredients_data:
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f"Missing required field {e} in item: {item}")
                        )
                    except IntegrityError:
                        self.stdout.write(
                            self.style.WARNING(f"Ingredient already exists: {item['name']}")
                        )
